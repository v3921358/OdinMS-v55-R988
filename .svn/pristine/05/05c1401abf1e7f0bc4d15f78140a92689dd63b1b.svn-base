...
public class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		Collection<IItem> items = c.getPlayer().getInventory(MapleInventoryType.CASH).list();
		for (IItem item : items) {
		    if (item.getItemId() >= 5000000 && item.getItemId() <= 5000045) {
			//log.info("Pet ID from item: {}", item.getPetId());
			try {
			    PreparedStatement ps = con.prepareStatement("SELECT * FROM pets WHERE petid = ?");
			    ps.setInt(1, item.getPetId());
			    ResultSet rs = ps.executeQuery();
			    rs.next();
			    //log.info("Pet ID; from item: {}, from db: {}", item.getPetId(), rs.getInt("petid"));
			    //log.info("Name: {}, Level: {}", rs.getString("name"), rs.getInt("level"));
			    //log.info("Closeness: {} Fullness: {}", rs.getInt("closeness"), rs.getInt("fullness"));
			    c.getSession().write(MaplePacketCreator.updatePet(item.getPosition(), item.getItemId(), rs.getInt("petid"), rs.getString("name"), rs.getInt("level"), rs.getInt("closeness"),  rs.getInt("fullness"), true));
			    rs.close();
			    ps.close();
			} catch (SQLException ex) {
			    java.util.logging.Logger.getLogger(PlayerLoggedinHandler.class.getName()).log(Level.SEVERE, null, ex.getCause());
			}
		    }
		}*/
