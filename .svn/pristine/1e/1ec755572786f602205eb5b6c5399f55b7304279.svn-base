		...
		if (status == 0) {
			...
			if (beauty == 1){
				if (cm.haveItem(5150010) == true){
					...
					cm.getChar().setHair(hairnew[Math.floor(Math.random() * hairnew.length)]);
					cm.getChar().updateSingleStat(MapleStat.HAIR, cm.getChar().getHair());
					cm.getChar().equipChanged();
			...
			if (beauty == 2){
				if (cm.haveItem(5151000) == true){
					...
					cm.getChar().setHair(haircolor[Math.floor(Math.random() * haircolor.length)]);
					cm.getChar().updateSingleStat(MapleStat.HAIR, cm.getChar().getHair());
					cm.getChar().equipChanged();
