	...
	if (mode == -1) {
		...
		if (status == 0) {
			if (!(cm.getJob().equals(MapleJob.HUNTER) || cm.getJob().equals(MapleJob.CROSSBOWMAN))) {
				if ((cm.getJob().equals(MapleJob.RANGER) || cm.getJob().equals(MapleJob.SNIPER)) && cm.getLevel() >= 120) {
					cm.sendOk("Please go visit #bLegor#k. She resides in #bLeafre#k.");
					cm.dispose();
					return;
				} else {
					cm.sendOk("You wouldn't believe what I can do with this bow...");
					cm.dispose();
					return;
				}
			}
			if ((cm.getJob().equals(MapleJob.HUNTER) || cm.getJob().equals(MapleJob.CROSSBOWMAN)) && cm.getLevel() >= 70 &&  cm.getChar().getRemainingSp() <= (cm.getLevel() - 70) * 3) {
			...
			if (cm.getJob().equals(MapleJob.HUNTER)) {
				...
				cm.getChar().gainAp(5);
				cm.sendOk("You are now an #bSniper#k.\r\n\r\nNow go, with pride!");
				cm.dispose();
			}
		}
	}
